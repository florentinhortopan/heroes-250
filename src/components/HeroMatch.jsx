...
const HeroMatch = ({
  ...
  const { rank, name, last_name, message_content } = hero;
  const { strength, value, traits = [], legacy } = message_content;
  ...
  const taglineFromAi = narrative?.tagline;
  const paragraphFromAi = narrative?.paragraph;
  const qualitiesCopy = Array.isArray(narrative?.qualitiesCopy) ? narrative.qualitiesCopy : [];
  const qualityLineFor = (id) => qualitiesCopy.find((q) => q.keyword === id)?.line;
    ...
    <section className="hero hero-secondary" data-version="hero-secondary">
      ...
      <div className="column-right">
        <div className="cmp-container">
          ...
          <div className="soldier-mobile-slot" aria-hidden="false">
            <SoldierDetail
              hero={{
                name,
                rank,
          ...
          <Accordion
            items={[{
              ...
              content: (
                <>
                  <p style={{ marginTop: 0, marginBottom: sharedQualities.length ? '1.25rem' : 0 }}>
                    {paragraphFromAi ? (
                      paragraphFromAi
                    ) : (
                      <>
                        ...
                        <li key={id} className="flex gap-2 my-6 first:mt-0 last:mb-0 rte">
                          <div className="icon icon-list flex shrink-0 w-[20px] h-[20px] mt-[3px]">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" height="20" viewBox="0 0 40 41" width="20">
                              <rect height="38" rx="19" stroke="#221F20" strokeWidth="2" width="38" x="1" y="1.98633"></rect>
                          ...
                          <div>
                            <p style={{ marginBottom: qualityLineFor(id) ? 4 : 0, fontWeight: 600 }}>{keywordLabelMap[id] || id}</p>
                            {qualityLineFor(id) && (
                              <p style={{ fontSize: '0.9rem', opacity: 0.85, margin: 0 }}>{qualityLineFor(id)}</p>
                            )}
                          </div>
                        </li>
                      ))}
          ...
          <Accordion
            items={[{
              ...
              defaultOpen: true,
              content: (
                  <CareerPath hero={hero} job={topJob} />
              )
            }]}
          />
          ...
          <div className="cta-block">
            <a href="/quiz" className="cta-secondary" onClick={handleRetake}>
