...
const JobCard = ({ job }) => {
  ...
  return (
    <section
      ...
      onClick={() => window.open(job.url_path_legacy, '_blank', 'noopener,noreferrer')}
      tabIndex={0}
      role="button"
    ...
    >
      <header className="card-header">
        ...
        <span className="job-group">{job.category && job.category.replace(/-/g, ' ')}</span>
      </header>
      <div className="card-content">
        <span className="job-title">{job.mos_title}</span>
        <span className="job-code">{job.moscode}</span>
        <span className="job-desc">{job.mos_description_short}</span>
      </div>
      <footer className="card-footer">
        <button className="save-btn" aria-label={`Save Job of ${job.mos_title} ${job.moscode}`}>{/* Save icon */}
