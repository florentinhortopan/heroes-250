...
// Pick the keywords the next question MUST use, one per option.
...
// server-side we get:
//  - Q1 and Q2: a randomized keyword sweep, so different users see
...
function pickRequiredKeywords({ history, snapshot, quizLength = 6, optionCount = 4 }) {
  ...
  if ((history?.length ?? 0) < broadenUntil) {
    return shuffleInPlace([...all]).slice(0, optionCount);
  }

  const out = [];
  // Start by covering fresh signal, then mix in probe keywords.
  const untalliedTarget = Math.min(optionCount, Math.max(1, Math.ceil(optionCount / 2)));
  for (const u of shuffleInPlace([...untallied])) {
    if (out.length >= untalliedTarget) break;
    out.push(u);
  }
  // Fill remaining slots with probe keywords to discriminate top heroes.
...
function buildNextQuestionSchema(optionCount) {
  return {
    name: 'next_question',
    schema: {
      type: 'object',
...
function nextQuestionSystemPrompt(optionCount) {
  return [
    ...
    '',
    'You will receive:',
    `- requiredKeywords: the exact ${optionCount} keywords you MUST use for this question, one per option, in any order. This is non-negotiable.`,
    ...
    '',
    'RULES:',
    `- Return ONE next question with EXACTLY ${optionCount} distinct options.`,
    `- Each option's keyword MUST be one of requiredKeywords. Use ALL ${optionCount} requiredKeywords, exactly one per option. Do not substitute or skip.`,
    ...
    `- HARD GUARDRAIL #2: NONE of the ${optionCount} option texts may match, paraphrase, or near-duplicate any entry in seenOptions. Same meaning expressed with the same key verb or noun is a duplicate. Vary verbs and framing.`,
    ...
    `- Options should read like natural responses, not labels. Never name the keyword in the option text. Mix sentence shapes (verb-led, noun-led, "I would..." style) so the ${optionCount} options don't feel parallel-stamped.`,
    `- ALWAYS set done=false and ALWAYS return a real question with exactly ${optionCount} options. The server is solely responsible for deciding when the quiz ends; you do not get a vote. If you set done=true, the server will discard your response and ask you again.`,
...
export async function handleNextQuestion(body) {
  ...
  const optionCount = Math.max(2, Math.min(ALLOWED_KEYWORDS.length, Number(body?.optionCount ?? 4)));
  ...
  const requiredKeywords = pickRequiredKeywords({ history, snapshot, quizLength, optionCount });
  ...
  const baseMessages = [
    { role: 'system', content: nextQuestionSystemPrompt(optionCount) },
    {
      role: 'user',
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (retryReason === 'premature_done') {
      messages.push({
        role: 'system',
        content: `Your previous draft set done=true. The server has already told you the quiz is NOT over (otherwise it would not have called you). Set done=false and return a real question with exactly ${optionCount} valid options NOW.`,
      });
    } else if (retryReason === 'duplicate') {
      messages.push({
        role: 'system',
        content: `Your previous draft repeated a question or option already shown this session. Generate a fresh stem AND ${optionCount} fresh option texts that do NOT paraphrase anything in seenQuestions or seenOptions.`,
      });
    }
    ...
    const completion = await client().chat.completions.create({
      ...
      response_format: { type: 'json_schema', json_schema: buildNextQuestionSchema(optionCount) },
      messages,
    });
    ...
    if (!parsed.question || !Array.isArray(parsed.options) || parsed.options.length < optionCount) {
      retryReason = 'premature_done';
      parsed = null;
      continue;
    }
  ...
  if (parsed && !parsed.done) {
    parsed.options = (parsed.options || []).slice(0, optionCount).map((o, i) => ({
      id: o.id || `opt_${i + 1}`,
      text: o.text,
      keyword: o.keyword,
    }));
